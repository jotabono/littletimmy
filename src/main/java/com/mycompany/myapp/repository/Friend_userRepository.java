...
public interface Friend_userRepository extends JpaRepository<Friend_user,Long> {
    ...
    @Query("select friend_user from Friend_user friend_user where friend_user.friend_to.login = :login " +
        "OR friend_user.friend_from.login = :login AND friend_user.friendship = true")
